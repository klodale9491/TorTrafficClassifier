...
# cartella 2 - tipologia di classificazione binaria
if os.path.isdir('datasets/csv/mix/samples/convolution/'+str(n_samples_gen)+'/binary') is False:
    os.mkdir('datasets/csv/mix/samples/convolution/'+str(n_samples_gen)+'/binary')

# cartella 3 - tipologia di classificazione multiclasse
...
for ds in datasets:
    ...
    df_label = pd.read_csv(base_read_path + 'multi/' + ds + '.csv', sep=',', header=None, dtype=col_dtypes)
    ...
    print(base_write_path + 'multi/' + ds + '.csv')
    pd.DataFrame(np.column_stack((features_label_trx, classes_label))).to_csv(base_write_path + 'multi/' + ds + '.csv', header=None, index=False)